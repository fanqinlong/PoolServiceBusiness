...
export function IViewSidebar({ currentPage, onPageChange, collapsed = false }: SidebarProps) {
  ...
  const renderMenuItem = (item: MenuItem, level = 0) => {
    ...
    if (hasChildren) {
      return (
        <div key={item.id}>
          <div
            className={`flex items-center px-4 py-3 cursor-pointer transition-all duration-200 text-sm ${
              isChildActive 
                ? 'text-[var(--sidebar-active-text)] bg-[var(--sidebar-active-bg)] border-r-2 border-[var(--primary)]' 
                : 'text-[var(--sidebar-text)] hover:text-[var(--sidebar-active-text)] hover:bg-[var(--sidebar-menu-bg)]'
            }`}
  ...
  return (
    <div className={`${collapsed ? 'w-16' : 'w-64'} h-full bg-[var(--sidebar-bg)] border-r border-[var(--sidebar-border)] flex flex-col transition-all duration-300`}>
      <div className="px-4 py-4 border-b border-[var(--sidebar-border)]">
        <div className="flex items-center gap-3">
          ...
          {!collapsed && (
            <div>
              <h1 className="text-[var(--foreground)] font-medium text-base">Pool Service Pro</h1>
              <p className="text-[var(--muted-foreground)] text-xs">Admin Dashboard</p>
            </div>
          )}
        </div>
      </div>
